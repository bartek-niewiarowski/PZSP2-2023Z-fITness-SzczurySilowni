...
class GymsTrainers(models.Model):
    gym = models.OneToOneField(Gyms, models.DO_NOTHING,
                               primary_key=True)  # The composite primary key (gym_id, trainer_id) found, that is not supported. The first column is selected.
...
class Users(models.Model):
    ...
    @staticmethod
    def get_number_of_all_subscriptions():
        all_users = Users.objects.all()
        # subs_types = SubscriptionPlans.objects.all()
        try:
            number_of_each_sub = {user.subscription_plan_id: all_users.count(user.subscription_plan_id) for user in
                                  all_users}
            return number_of_each_sub
        except ValueError:
            return {}
